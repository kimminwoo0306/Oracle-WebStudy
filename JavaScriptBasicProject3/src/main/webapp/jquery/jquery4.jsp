...
<%--
		1. selector => CSS => $('tag_name') $('#id명') $('.클래스명') $(내장객체) $(function)
		              ...
		              window : open(팝업) => 아이디 중복 체크, 우편번호 검색, close()
		...
		2. jquery 함수
		   ...
		   = 태그의 값을 설정 (setter)
		     ...
		     <span></span> ==> $('span').text("<font color=red>Hello</font>")
		           <span><font color=red>Hello</font></span>
		                 ---------------------------- 문자열처리
		           $('span').html("<font color=red>Hello</font>")
		      --------------------------------------------------------setter : 값을 한번만, append()를 이용해서 여러번 사용
		   = 상태
		     $(selector).focus()
		     $(selector).hide()
		     $(selector).show()
...
$(function(){
	/* $('span').html("<font color=red>Hello JavaScript</font>") */
	/* $('span').text("1");
	$('span').text("2"); */
	/* $('span').append("1")
	$('span').append("2") */
 			...
 			<tr>
 				...
 				<td width=80%>
 				  <table class="table">
 				    <tr>
 				      <td class="text-right">
 				      	<%-- id, class(css,javascript제어),name (자바에서 값을 받는 경우) --%>
