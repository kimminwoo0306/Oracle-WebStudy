...
public class HTMLParser {
	...
	public void htmlGetData()
	{
		...
		/*	Elements h1=doc.select("div.b h1.f1");
	...
	public void foodDetailData()
	{
		try
		{
			...
			Elements h1=doc.select("table.info tbody tr th");
			String address="", tel="", type="", price="", parking="", time="", menu="";
	
			for(int i=0;i<h1.size();i++)
			{
				//System.out.println(h1.get(i).text());
				String label=h1.get(i).text();
				if(label.equals("주소"))
				{
	...
	public void scriptData()
	{
		try
		{
			...
			/*
			 * 1. HTML데이터 추출 => HTMLParser => Jsoup
			 * 2. JSON 데이터 추출 => JSONparser (Ajax, Vue, React)
			 *    => ArrayList => []
			 *    => ~VO       => {}
			 * 3. XML 데이터 추출 => DocumentBuilder (Spring, Mybatis)
			 * 
			 * [] => JSONArray
			 * {} => JSONObject
			 */
