...
public class ClientMain extends JFrame implements ActionListener, Runnable{
	...
	private BufferedReader in; // 서버에서 보내준 값을 받는다 ==> 쓰레드이용  ==> inreadLine()
	...
	public ClientMain()
	{
		...
		setSize(1150, 850);
		setVisible(true);
		setDefaultCloseOperation(EXIT_ON_CLOSE); // X 눌렀을때 메모리 해제
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==login.b1)
		{
			...
			String sex="";
			if(login.rb1.isSelected()) // 남자 라디오 버튼이 선택되었을때
			{
				sex="남자";
			}
			else
		...
		else if(e.getSource()==wr.tf)
		{
			...
			if(msg.trim().length()<1) // 입력이 안됬을 때
				return; // 메소드 종료
			try
			{
	 ...
	 *        예) 로그인(id,pwd), 영화 목록(페이지 번호), 영화 상세 (중복이 안된 데이터)...
	...
	public void connection(String id, String name, String sex)
	{
		...
		// 로그인 요청 => 사용자 정보 (id, name, sex) => 로그인 요청, 서버에서 결과값이 있는 경우 => 받아서 출력
		try
		{
			// 연결
