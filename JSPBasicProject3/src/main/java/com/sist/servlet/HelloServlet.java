...
@WebServlet("/hello.sist")
// 클라이언트 (브라우저) <=======> 서버
//                     주소창 (연결이 가능)
// 사용자가 URL 주소 ==> 마지막에 /HelloServlet ==> 톰캣이 HelloServlet을 호출
/*
 * 		클라이언트에서 어떤 값을 전송할지 ... (Front End)
 * 		요청값을 받아서 어떤 화면을 브라우저로 전송 할 지 (Back End)
 ...
 *           
 *      서블릿 호출
 *      1) http://localhost:8080/JSPBasicProject3/HelloServlet 브라우저에서 요청
 *      2) 서버에서 ==> WAS에서 서블릿 클래스를 찾는다 (HelloServlet)
 *                    ---Web Application Server (톰캣, 레진) ==> WAS(톰캣), 형상관리 (git)
 *      3) init()메소드를 호출
...
public class HelloServlet extends HttpServlet {
	 ...
	 *          POST : no=10 ==> 감춰서 보내준다 (URL뒤에 데이터가 안보인다)
	 *            ==> 보안 (ID, Password, 데이터가 많은 경우)
	 */
	// 브라우저 화면을 출력하는 메소드
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		// 1. GET/ POST가 지정되지 않는 경우에는 GET 이 호출된다 (default)
		/*
		 *   서버로 전송
		 *   --------
		 *   HTML
		 ...
		 *         axios.get(), axios.post()
		 *   Java
		 *      sendRedirect() => GET방식만 가능
		 */
		//1. 브라우저에 보내는 형식을 지정 ==> text/html, text/xml, text/plain
		//                                 HTML       XML       JSON
		response.setContentType("text/html;charset=UTF-8"); // 한글을 포함
		//2. 어떤 브라우저로 전송할지 설정
