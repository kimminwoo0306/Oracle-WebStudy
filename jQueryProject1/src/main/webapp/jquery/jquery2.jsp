...
$(function(){
	...
	/*
		getter/setter => 태그값 읽기, 속성값 읽기 (저장된 데이터값을 읽는 경우)
		=============
			      <input>, <select>, <textarea> : val():읽기, val(값):쓰기
			      <td>값</td> : text():읽기 , text(값): 쓰기 => 모든 태그에 적용
			                    html():읽기, html(값):쓰기
			      <a href="">, <img src=""> => attr("속성명") => $('img').attr("src")
			                                   attr("속성명","값")
			      여러개 동시에 첨부 : append
	    
	     이벤트 사용법
	       click : button, img, span ....
	       $('선택자').click(function(){
	    	   처리
	       })
	       ...
	       ----------------------------------------------
	       show(), hide()
	       show(2000) 2초 , show(fast) => 속도 (이펙트)
	*/
	$('#keyword').keyup(function(){
		let k=$('#keyword').val(); // 입력한 값
		$('#user-table tbody tr').hide() // 감추기
		let temp=$('#user-table tbody tr td:nth-child(5n+3):contains("'+k+'")')
		$(temp).parent().show() // 보여주기
		// td찾고 => 출력 tr => CSS를 이용
	})
})
</script>
