        ...
        => 저장된 데이터 (이미지:데이터를 볼 수 없다(보안), 하드디스크에 저장)
                            응용프로그램(JDBC)
                              ...
                              오라클 
                               |
                            하드디스크 
        => 운영데이터 : 사이트에 필요한 데이터 검색할 목적으로 저장된에 접근 (공유)
        => 공유된 데이터 : 접속자 모든 사람이 모든를 공유할 목적 (오라클 : static) => 팀별 프로젝트 => 오라클 1개만 사용 
    ...
    3. 시스템의 구성 요소 (27page)
       ...
       => 키에 대한 종류 
            = 기본키 (중복을 허용하지 않는 키) => ID,대부분은 숫자로 저장 (자동 증가) =>  PRIMARY KEY(모든 테이블은 기본키를 1개 이상 포함)
                                              무결성의 원칙 
            = 후보키 : 기본키를 대체하는 키 (email,tel) => 주민번호 (사용금지=>공기업,은행) => UNIQUE(null갑을 허용)
    ...
    5. 45page
         응용프로그램 (자바)
           ...
           오라클 
             | read/write
    하드디스크(실제 저장된 데이터)
    6. 46page => 데이터베이스 언어 (SQL) => 문자열로 되어 있다 
       ----------
       SQL 종류 
       1) DML (데이터 조작 언어) 
          = DQL 
