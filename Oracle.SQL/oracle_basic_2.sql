...
/*
     *** 오라클
         1. 대소문 구분을 하지 않는다 (단 데이터값을 대소문자를 구분한다)  => 명령문은 대소문자를 구분하지 않는다 
     ...
     1) SELECT => 데이터를 검색 (추출)
        = 형식 
          SELECT * | column1,column2...  => 전체 데이터(*) , 출력에 필요한 데이터만 추출 
          FROM table_name
          [
...
SELECT DISTINCT deptno FROM emp; -- 장르 ,부서명 , 근무지 ....
/*
     1) 형식 
        = 전체 데이터 추출  ==> SELECT *
