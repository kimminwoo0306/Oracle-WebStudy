...
/*
  비등가 조인 (NON EQUI_JOIN) => =이외의 연산자 사용시
*/
...
-- emp => empno, ename, job, hiredate, sal => dname, loc ==> SCOTT사원의 정보
-- 사원정보, 부서정보 => 두 테이블 필요한 데이터를 추출 ==> JOIN
...
/*
   162page
     => 2 이상의 테이블에서 데이터값 읽기 vuerd => 데이터베이스 설계 => dbleaver
       1) JOIN : 2개 이상의 테이블에 데이터를 추출하는 방법
                == 165page
                   여러개의 테이블을 연결해서 하나의 테이블을 만들어가는 과정
                   동등조인(EQUI JOIN) ==> 연산자 사용시에 =를 사용한다
                   WHERE 테이블명.컬럼명=테이블명.컬럼명
                   ------------------------------- 테이블과 테이블이 같이 값을 저장하고 있어야 조인이 가능
                -  ------------------------------- 단점 : null을 가지고 있으면 검색할 수 없다(null을 포함하지 않는다)
                   ...
                   aaa 이름 나이    aaa B
                   셀프조인 => 별칭
                   구분 => 테이블, 별칭 (NATURAL JOIN, JOIN USING 들은 별칭 사용 x)
       2) SubQuery : 2개 이상의 SQL을 연결해서 처리
         
        168page => OUTER JOIN (null을 포함해서 데이터를 가지고 온다)
