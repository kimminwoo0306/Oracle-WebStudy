     ...
     145page 
       ...
       *** 참조 ==> FROM table_name 
       ...
       1) 산술연산자 => 주로 SELECT 뒤에 붙여서 사용 
          + , - , * , / ==> 문자열을 연산차리하면 오류 발생 ==> ||
                        ==> /는 0으로 나눌수 없다 , 정수/정수=실수 
                        ==> 컬럼의 값이 없는 경우 => NULL 은 연산자처리가 안된다 
                        null + 1 = null
       ----------------------조건 검색시에 사용 
...
SELECT ' 10'+10,10-2,10*2,10/3 
FROM DUAL;
-- trim() 가능 ==> 산술연산자 => 통계(합) , 평균 
-- NULL값인 경우에 다른 값을 대체
...
/*
           AND : 직렬
           (조건) AND (조건)   => true and true => true가 되는 연산자 ==> 범위,기간이 포함시 처리 
           OR : 병렬
           (조건) OR (조건) => 둘중에 1개가 true면 true가되는 연산자 => 범위밖에 있는 경우 
...
-- 부서번호(deptno)가 30번인 사원의 모든 정보를 가지고 온다 , 20사원 포함
SELECT *
FROM emp
WHERE deptno=30 OR deptno=20;

-- NULL 연산자 ==> 값null일경우에는 연산처리가 안된다 
...
-- IN연산자 => OR가 여러개 일 경우에 처리 
-- 급여가 5000 , 2850 , 1500 , 3000 ,950을 받는 사원의 모든 정보 
SELECT *
FROM emp
WHERE NOT  (sal=5000 OR sal=2850 OR sal=1500 OR sal=3000 OR sal=950);

SELECT *
FROM emp
WHERE sal NOT IN(5000,2850,1500,3000,950);
-- 문자 
...
-- 연산자중에 가능 많이 사용되는 연산자 ==> LIKE ==> 모든 검색기 
