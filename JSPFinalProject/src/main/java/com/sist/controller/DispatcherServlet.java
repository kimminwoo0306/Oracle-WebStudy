 ...
 *  MVC 동작 과정
 *  
 *  1. 요청 ==> DispacherServlet을 찾는다
 *           --------------------- URL 주소로 찾는다 => .do (list.do, insert.do, delete.do => DispacherServlet)
 *      ** Controller : 요청을 받는다 => Model로 전송(요청내용 전송 => request)
 *                      ------------------------
 *                      Model처리 결과를 받는다 (request) => request.setAttribute()(여러개를 사용할 수 있다)
 ...
 *                     
 *           Controller 역할
 *           1) 사용자 요청 => 해당 Model클래스를 찾는다 ==> 등록된 Model클래스
 *                          --------------------- 등록 (XML) => Spring
 *           2) 요청 처리 ==> Model 메소드 호출 => invoke
 *           3) Model 메소드 호출 후 => 결과값 받는다 (request, session)
 *           4) JSP를 찾아서 request를 전송 => forward()
 */
@WebServlet("*.do")
public class DispatcherServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	ArrayList<String> clsList=new ArrayList<String>(); // application.xml에 등록된 클래스를 모아둔다 -> Model 클래스 인식
	
	// 초기화 => 사용자에게 서비스 (JSP로 request => 화면 브라우저에 출력)
	public void init(ServletConfig config) throws ServletException {
		// Model 클래스를 읽어온다 => 일반 Window형식 => 경로 : 1)Git 2) AWS(리눅스)
		try
		{
			// 1. XML파일 가지고 온다
			// 1-1. xml의 파일 읽기 => 호환성 => 실제 톰캣이 인식하고 있는 폴더에서 읽기
		...
		try
		{
			...
			for(String cls:clsList)  // 등록된 클래스안에서 해당 요청의 메소드를 호출한다
			{
				...
				for(Method m:methods)
				{
					...
					{
						// 찾은 경우에 => 메소드를 호출해라
						String jsp=(String)m.invoke(obj, request,response); // 메소드명은 상관없이 어노테이션이 일치하는 메소드 호출할 수 있다
						// request => JSP로 전송
