 ...
 *        = Row 1개일 때
 *          rs.next(); 메모리에 출력된 데이터 첫번째 줄에 커서 위치
 *        = Row가 여러개
 *          while(rs.next())
 *     7. 처리 종료
 *        rs.close()
 ...
 *           
 *        = 코딩 순서
 *        1) 미리 생성된(톰캣) Connection의 주소값을 얻어 온다
 *           1. Connection객체 주소가 저장된 위치로 접근 시도
 *              java://comp/env => 저장되는 위치
 *              jdbc/oracle
 *              
 *              Context init=new InitialContext(); => 접근
 ...
 *            conn.close()
 *            
 *         JDBC / DBCP ==> Connection : 매번 설정 / 미리 생성
 */
...
public class DataBoardDAO {
	 ...
	 *     2) 결과값을 어떤값 => 리턴형
	 *        = 목록 (ArrayList)
	 *        = 상세 (~VO) => 게시물 한개에 대한 모든 데이터 변수를 가지고 있따
	 *        = 일반
	 *     3) 메소드를 구현
	 *     
	...
	public ArrayList<DataBoardVO> databoardListData(int page)
	{
		...
		try
		{
			// 1. Connection 주소 얻기
			getConnection();
			// 2. SQL 문장 ==> index => UNIQUE, PRIMARY KEY => 자동 생성
		...
		try
		{
			...
			if(db_pwd.equals(vo.getPwd()))
			{
				...
				sql="UPDATE databoard SET "
				   +"name=?, subject=?, content=? " // moddate=SYSDATE
				   +"WHERE no=?";
				ps=conn.prepareStatement(sql);
				ps.setString(1, vo.getName());
				ps.setString(2, vo.getSubject());
