...
public class SeoulDAO {
	// 연결 객체 => 미리 생성된 connection객체의 주소를 가지고 온다
	private Connection conn;
	// 오라클과 송수신 (SQL전송 == 결과값 얻기)
	private PreparedStatement ps;
	// DBCP ==> 요청시마다 생성(Connection이 많이 생성), 미리연결된 객체를 사용
	private static SeoulDAO dao; // 싱글턴 패턴 (사용자마다 한개의 DAO만 사용이 가능하게 만든다)
	// DBCP의 단점 => 웹에서만 사용이 가능 (톰캣)
	...
	public void getConnection()
	{
		try
		{
			...
			// 루트 경로 이용
			Context c=(Context)init.lookup("java://comp/env"); // c드라이브 접근한다
			// Connection 객체를 얻어 온다
			DataSource ds=(DataSource)c.lookup("jdbc/oracle"); // 해당 파일을 찾는다
			// 주소
	 ...
	 *      톰캣에 의해서 Connection을 10(maxIdle)개를 생성해서 => POOL이라는 메모 공간에 저장을 해둠
