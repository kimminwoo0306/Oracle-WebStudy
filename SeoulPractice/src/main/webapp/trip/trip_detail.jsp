...
<div class="container">
      ...
      <div class="nearbyFood_section">
          ...
          <div class="flexslider carousel basiccarousel btmspace-80">
            <ul class="slides">
              <c:forEach var="fvo" items="${flist }" varStatus="s"><%-- list의  index번호 --%>
                 <li>
                   <figure><img class="radius-10 btmspace-10" src="${fvo.poster }">
                     <figcaption class="text-center"><a href="../food/food_detail.do?fno=${fvo.fno }">${fvo.name }</a></figcaption>
                   </figure>
                 </li>
             </c:forEach>
            </ul>
...
<%--             <c:if test="${reviewCnt>0}">
         ...
         <div class="tReview_body">
            ...
            <!-- 댓글 : 수업시간 코드 활용 -->
              ...
              <!-- 댓글쓰기/리뷰등록 -->
             <h3><span style="color:#004fff;font-size:24px;">&nbsp;${vo.name }</span>에 대한 방문후기를 들려주세요!</h3>
              ...
              <c:if test="${count==0 }"> <!-- count=rList.size() 특정상세페이지의 덧글개수 -->
               <table class="table">
                 <tr>
                  <td class="text-left">아직 등록된 한줄리뷰가 없습니다.</td>
                 </tr>
               </table>
              </c:if>
...
<%--                        <tr>
                         ...
                         <td class="text-right" width=15%>
                           <c:if test="${sessionScope.id!=null }">
                             <c:if test="${sessionScope.id==rvo.id }">
                               <span class="btn btn-xs btn-danger ups" data-no="${rvo.arno }">수정</span>
                               <a href="../all_reply/all_reply_delete.do?arno=${rvo.arno }&no=${vo.tno}&cate_no=1" class="btn btn-xs btn-primary">삭제</a>
                             </c:if>
                           </c:if>
                         </td>
                       </tr> 
