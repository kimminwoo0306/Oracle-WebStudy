...
--%>
<%
   // 1. 사용자가 보내준 데이터를 읽는다 (page) > 이전, 다음 누를 때마다 페이지를 넘겨줌 
   ...
   */
   if(strPage==null)
      strPage="1"; // default 값 설정
   // 3. 현재 페이지 지정
   int curpage=Integer.parseInt(strPage);
   // 4. 현재 페이지에 해당되는 데이ㅓ를 오라클에서 갖고 온다
   BoardDAO dao=new BoardDAO();
   ArrayList<BoardVO> list=dao.boardListData(curpage);// 페이지를 줄 때 마다 내용이 다르게 들어옴 
   // 5. 총페이지 읽기
