...
public class BoardDAO {
   ...
   public int boardTotalPage()
   {
      ...
      try
      {
         ...
         ps=conn.prepareStatement(sql);
         ResultSet rs=ps.executeQuery(); // 데이터 값 읽기 : SELECT 
         rs.next();
         total=rs.getInt(1);
         rs.close();
      }catch(Exception ex)
   ...
   // 목록 출력 : 페이징 (인라인뷰), 출력 순서 (없어지는 번호... 어떻게?), 블록별 ([1][2][3]...) 
   // 상세 보기 : rownum (이전/다음)
   ...
   // 수정 => 비번이 일치가 되는지 안되는지? => javaScript
