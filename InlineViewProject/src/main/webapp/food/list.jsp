...
<%
	...
	if(strPage==null) // 맨 처음에는 선택을 못하기 때문에 1페이지로 지정
	{
		strPage="1";
	}
	int totalpage=dao.foodTotalPage(); // 총페이지
	int curpage=Integer.parseInt(strPage);	// 현재페이지 (사용자 요청에 의한)
	ArrayList<FoodVO> list=dao.foodListData(curpage); // 현재페이지에 대한 데이터를 오라클로부터 읽어온다
